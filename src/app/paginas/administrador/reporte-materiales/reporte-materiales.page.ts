...
import { ReporteMaterialesService } from '../../../procesos/reporte-materiales.service';
...
@Component({
  ...
  templateUrl: './reporte-materiales.page.html',
  styleUrls: ['./reporte-materiales.page.css'],
  standalone: true,
  imports: [
    CommonModule,
    IonicModule,
...
export class ReporteMaterialesPage {
  private navCtrl = inject(NavController);
  private toastCtrl = inject(ToastController);
  private cdr = inject(ChangeDetectorRef);

  private dashboardAdminService = inject(DashboardAdminService);
  private reporteMaterialesService = inject(ReporteMaterialesService);

  private readonly logoReportePath = 'assets/img/logo.png';
  
  adminVm$ = this.dashboardAdminService.obtenerPanelAdmin$();
  vm$: Observable<ReporteMaterialesVM> = this.reporteMaterialesService.vm$;

  filtros: {
    valor: FiltroReporteMateriales;
    texto: string;
  }[] = [
  ...
  cambiarFechaInicio(valor: string) {
    this.reporteMaterialesService.cambiarFechaInicio(
      this.normalizarFechaInput(valor)
    );
  }

  cambiarFechaFin(valor: string) {
    this.reporteMaterialesService.cambiarFechaFin(
      this.normalizarFechaInput(valor)
    );
  }
  ...
  trackByMaterial(
    index: number,
    material: ReporteMaterialVista
  ): string {
    return material.uid || String(index);
  }

  private obtenerResumenPDF(materiales: ReporteMaterialVista[]) {
    return {
  ...
  private dibujarPiePaginasPDF(doc: jsPDF) {
    ...
    for (let pagina = 1; pagina <= totalPaginas; pagina++) {
      ...
      doc.text(
        'Control operativo | Reporte de materiales',
        14,
        pageHeight - 8
      );
